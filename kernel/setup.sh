...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU-Next" || git clone https://github.com/imnathanzero/KernelSU-Next && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU-Next"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout susfs-4.9 && echo "[-] Switched to next branch."
    fi
